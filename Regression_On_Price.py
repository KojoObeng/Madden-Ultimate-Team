...
class model:
    def __init__(self):
        ...
        self.skills_picked = ['SPD', 'AWA', 'CTH']

        def add_categorical_features():
            self.MutData = pd.get_dummies(data=self.MutData, columns=['POS', 'PROGRAM'], drop_first=True)

        def add_quadratic_features():
            for skill in self.skills_picked:
                self.MutData[skill + " SQUARED"] = self.MutData[skill].apply(lambda x: x**2)

        def add_pos_interactions():
            pos_skill_dict = [[["POS_QB"], ['RUN']],
                               [["POS_RT", "POS_RG", "POS_LG", "POS_LT"], ["RBK", "PBK", ]],  # "RBP", "RBF", "PBP", "PBF"
        ...
        def add_speed_interactions():
            ...
            speed_pos = [col for col in self.skills_picked if ("POS_" in col) and col not in []]
        ...
        def drop_skills():
            self.MutData.drop(np.setdiff1d(self.all_skills, self.skills_picked), axis=1, inplace=True)
    ...
    def do_regression(self):
        ...
        def setup_regression():
            ...
            self.X = self.MutData.drop(columns=["PLAYER_NAME", "TEAM", "PRICE"], inplace=False) # ,"POS", "PROGRAM"
        ...
        def ridge_regression():
            # Create regression
            self.ridge_regr = linear_model.Ridge(alpha=2.5).fit(self.X_train, self.y_train)
        ...
        def lasso_regression():
            # Create regression
            self.lasso_regr = linear_model.Lasso(alpha=2.5).fit(self.X_train, self.y_train)
